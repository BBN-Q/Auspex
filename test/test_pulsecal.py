...
class SingleQubitCalTestCase(unittest.TestCase):
    ...
    * RamseyCalibration
    ...
    def sim_ramsey(self, set_source = True):
        ...
        """
        ideal_data = [np.tile(simulate_ramsey(detuning = 90e3), self.num_averages), 
                      np.tile(simulate_ramsey(detuning = 45e3), self.num_averages)]
        ramsey_cal = RamseyCalibration(self.q, num_steps = len(ideal_data[0])/(self.num_averages),
                        added_detuning = 0e3, delays=np.linspace(0.0, 50.0, 50)*1e-6, 
                        set_source = set_source)
        ramsey_cal.set_fake_data(self.x6_1, ideal_data, increment=True)

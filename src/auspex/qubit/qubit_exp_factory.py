...
class QubitExpFactory(object):
    ...
    def create(meta_file):
        ...
        # Build a mapping of qubits to receivers. Only receivers mentioned in the metafile 
        # actually receive the output of MEAS pulses. 
        receivers = [rcv.replace('RecvChan-', '') for rcv in self.meta_info["receivers"]]
        qubits_by_receiver = {rcv: rcv.replace('-SS', '') for rcv in receivers}
        # qubits_by_receiver = {v["measure"]["receiver"]: k for k, v in settings["qubits"].items()}
        qubits = list(qubits_by_receiver.values())
        self._qubit_proxies = {q: QubitProxy(self, q) for q in qubits}

        # Find out which instruments are available
        instruments = settings["instruments"]
        stream_info_by_qubit = {q: settings["qubits"][q]["measure"]["receiver"].split() for q in qubits}
        digitizer_settings_by_qubit = {q: instruments[stream_info_by_qubit[q][0]] for q in qubits}
        
        for q, qp in self._qubit_proxies.items():
            qp.digitizer_settings = digitizer_settings_by_qubit[q]
            qp.available_streams = available_digitizers[qp.digitizer_settings['type']].available_streams

...
class QubitExperiment(Experiment):
    ...
    def final_init(self):
        ...
        self.outputs_by_qubit =  {q.label: [self.proxy_to_filter[dat['node_obj']] for f,dat in self.modified_graph.nodes(data=True) if isinstance(dat['node_obj'] and f == q.label, (bbndb.auspex.Write, bbndb.auspex.Buffer,))] for q in self.measured_qubits}

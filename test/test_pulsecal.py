...
def simulate_drag(deltas = np.linspace(-1,1,11), num_pulses = np.arange(16, 64, 4), drag = 0.6):
    """
...
class SingleQubitCalTestCase(unittest.TestCase):
    ...
    * RamseyCalibration
    ...
    def test_drag(self):
        ...
        ideal_data = [np.tile(simulate_drag(drag = ideal_drag), self.nbr_round_robins)]
        np.save(self.filename, ideal_data)
        drag_cal = cal.DRAGCalibration(self.q.label)
        cal.calibrate([drag_cal])
        import ipdb; ipdb.set_trace()
        # Run a second time with smaller drag step
        ideal_data = [np.tile(simulate_drag(ideal_drag), self.nbr_round_robins)]
        np.save(self.filename, ideal_data)
        os.remove(self.filename)

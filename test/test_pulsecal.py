...
def simulate_drag(deltas = np.linspace(-1,1,21), num_pulses = np.arange(16, 48, 4), drag = 0.6):
    ...
    xpoints = np.arange(len(deltas)*len(num_pulses))
    #ypoints = np.tile((deltas - drag)**2, len(num_pulses))
...
class SingleQubitCalTestCase(unittest.TestCase):
    ...
    * RamseyCalibration
    ...
    def test_drag(self):
        ...
        """
        ideal_drag = 0 # arbitrary choice for testing
        ideal_data = [np.tile(simulate_drag(drag = ideal_drag), self.nbr_round_robins), np.tile(simulate_drag(drag = ideal_drag, deltas = np.arange(ideal_drag - 0.5, ideal_drag + 0.5, 21), num_pulses = np.arange(32, 96, 8)), self.nbr_round_robins)]
        np.save(self.filename, ideal_data)
        drag_cal = cal.DRAGCalibration(self.q.label)
        opt_drag = cal.calibrate([drag_cal])[2]

        os.remove(self.filename)
        self.assertAlmostEqual(drag_cal, ideal_drag, places=2)
        #test update_settings
        new_settings = auspex.config.load_meas_file(cfg_file)
        self.assertAlmostEqual(opt_drag, new_settings['qubits'][self.q.label]['control']['pulse_params']['drag_scaling'],places=2)
        #restore original settings
        auspex.config.dump_meas_file(self.test_settings, cfg_file)

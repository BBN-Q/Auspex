...
class SingleShotMeasurement(Filter):
    ...
    force_demodulation = BoolParameter(default=False)
    demod_frequency = FloatParameter(default=100e6)
    ...
    def __init__(self, save_kernel=False, optimal_integration_time=False,
                    ...
                    logistic_regression=False, force_demodulation = False,
                    demod_frequency=100e6, **kwargs):
        ...
        if len(kwargs) > 0:
            ...
            self.force_demodulation = force_demodulation
            self.demod_frequency = demod_frequency
        ...
        self.quince_parameters = [self.save_kernel, self.optimal_integration_time,
            ...
            self.force_demodulation, self.demod_frequency]
    ...
    def compute_filter(self):
        ...
        if self.force_demodulation:
            self.ddc = np.exp(-2j*np.pi * self.demod_frequency * self.time_pts)
            self.ground_data = (self.ground_data.T * self.ddc).T
            self.excited_data = (self.excited_data.T * self.ddc).T
    ...
    def _save_kernel(self):
        if self.force_demodulation:
            self.kernel = np.multiply(self.ddc, self.kernel)
        try:

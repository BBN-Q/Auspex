...
if __name__ == '__main__':
    ...
    proc = Switching()
    proc.supply_current.value = 4.0

    # Define a sweep over prarameters
    sw = Sweep(proc)
    sw.add_parameter(proc.pulse_voltage, -7.5*np.power(10,-np.arange(9,8,-1)/20.0))
    sw.add_parameter(proc.pulse_duration, np.arange(0.2e-9, 0.31e-9, 0.1e-9))
    sw.add_parameter(proc.attempt_number, np.arange(0,50))

    # Define a writer
    sw.add_writer('SwitchingAttempts.h5', 'AmplitudeSearch-Neg-8', proc.initial_state, proc.final_state)

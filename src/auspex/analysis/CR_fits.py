...
class SineFit(AuspexFit):
	...
	def __init__(self, xpts, ypts, initial_phase, inital_freq):
...
def fit_CR_length(xpoints, data0, data1):
	...
	fit0 = SineFit(xpoints, data0, 1/(2.0*xpoints[-1]), np.pi/2.0) 
	fit1 = SineFit(xpoints, data1, 1/(2.0*xpoints[-1]), np.pi/2.0)

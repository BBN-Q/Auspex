...
class SingleQubitCalTestCase(unittest.TestCase):
    ...
    * RamseyCalibration
    ...
    def run_ramsey(self, set_source = True):
        ...
        """
        self.true_qubit_freq = 5.100065e9 
        added_detuning = 50e3
        qubit = cl['q1']
        qubit.frequency = 100e6
        detuning = qubit.frequency + qubit.phys_chan.generator.frequency + added_detuning - self.true_qubit_freq
        if detuning < 0:
            detuning2 = -0.5*detuning
        else:
            detuning2 =  1.5*detuning
        ideal_data = [simulate_ramsey(num_steps = 50, detuning = abs(detuning)),  
                      simulate_ramsey(num_steps = 50, detuning = detuning2)]
        ramsey_cal = RamseyCalibration(cl["q1"], do_plotting=False, added_detuning = added_detuning, 
                        delays=np.linspace(0.0, 49.0, 50)*1e-6, set_source = set_source)
        ramsey_cal.set_fake_data(cl["X6_1"], ideal_data, increment=False) # this keyword is meaningless if exp is recreated
    ...
    def test_ramsey_set_source(self):
        ...
        self.assertAlmostEqual(1e-9*ramsey_cal.fit_freq,1e-9*self.true_qubit_freq,places=5)
        expected_source_GHz = 1e-9*(self.true_qubit_freq-ramsey_cal.qubit.frequency)
        self.assertAlmostEqual(1e-9*ramsey_cal.qubit.phys_chan.generator.frequency,expected_source_GHz,places=6)
    ...
    def test_ramsey_set_qubit(self):
        ...
        self.assertAlmostEqual(1e-9*ramsey_cal.fit_freq,1e-9*self.true_qubit_freq,places=5)
        expected_qbf_MHz = 1e-6*(self.true_qubit_freq-ramsey_cal.qubit.phys_chan.generator.frequency)
        self.assertAlmostEqual(1e-6*ramsey_cal.qubit.frequency,expected_qbf_MHz,places=3)
